...
public class ExcelUtil {
	//excel表信息
    public static final String EXCEL_PATH = "src/test/resources/api_v5.xlsx";//用例表格excel文件路径
	...
	//定义一个存储接口返回数据对象的集合用来批量回写数据到excel中
	...
	public static void batchWriteBackDatas(String excelPath) {
		...
		try {
			...
			for(WriteBackData writeBackData:writeBackDatas){
				...
				String actual = writeBackData.getActualResult();
				//获取指定sheet页对象
				Sheet sheet = workbook.getSheet(sheetName);
				//根据行标识符获取行号并获取该行对象
